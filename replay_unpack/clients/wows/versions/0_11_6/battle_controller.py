...
class BattleController(IBattleController):
    ...
    def _times_to_win(self):
        ...
        for cap in self._getCapturePointsInfo():
            if cap['isEnabled'] and not cap['bothInside'] and cap['teamId'] != -1:
                if cap['teamId'] == self._owner['teamId']:
                    ally_tick += reward
                else:
                    enemy_tick += reward

        ally_ttw = self._time_to_win(ally_tick, period, self._win_score - self._dict_score[0].score)
        enemy_ttw = self._time_to_win(enemy_tick, period, self._win_score - self._dict_score[1].score)
